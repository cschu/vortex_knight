process read_counter {
    ...
    """
    mkdir -p ${sample.id}
	read_counter map -t $task.cpus -v 6 -db \$(readlink ${marker_db}) -l ${params.read_counter_min_length} ${params.read_counter_count_mode} ${read_counter_input} > ${sample.id}/${sample.id}.read_counter.txt
    """
}
