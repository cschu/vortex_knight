process motus {
    ...
    """
    mkdir -p ${sample.id}
    motus profile -t $task.cpus -k ${params.motus_tax_level} -c -v 7 -l ${params.motus_min_length} -g ${params.motus_n_marker_genes} -db ${motus_db} ${motus_input} > ${sample.id}/${sample.id}.motus.txt
    """
}
